    ...
    public static class ServerHandler implements IMessageHandler<PlayerConnectRadioPacket, IMessage> {
        ...
        public IMessage onMessage(PlayerConnectRadioPacket message, MessageContext ctx) {
            ...
            if(frequency<1 || frequency>1000){
                return new RadioResponsePacket((short) -1, RadioResponsePacket.ResponseCode.CONNECT_INVALID_FREQUENCY);
            }
            else {
                ...
                if(connectResponse==1) return new RadioResponsePacket(frequency, RadioResponsePacket.ResponseCode.CONNECT_BLACKLISTED);
                else if(connectResponse==2) return new RadioResponsePacket(frequency, RadioResponsePacket.ResponseCode.CONNECT_NO_PERM);

                RadioManager.connectPlayerTo(player, frequency);
            }
            return new RadioResponsePacket(frequency, RadioResponsePacket.ResponseCode.CONNECT_SUCCESS);        }
    }
}